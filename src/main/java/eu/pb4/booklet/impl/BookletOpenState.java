...
public record BookletOpenState(List<Identifier> previousPages) {
    ...
    public Dialog getDialog(Component title, List<DialogBody> body) {
        ...
        ).setStyle(UiResourceCreator.STYLE)).append(title);
        ...
        if (this.previousPages.isEmpty()) {
            ...
            var backButton = new ActionButton(new CommonButtonData(CommonComponents.GUI_BACK, buttonWidth), Optional.of(new StaticAction(BookletImplUtil.encodeClickEvent("open_page", pop.page, pop.state))));
